...
const TrackingPlanCreatedByMe = () => {
  ...
  const [personalTrackingPlansError, setPersonalTrackingPlansError] = useState("")
  const [personalTrackingPlansNext, setPersonalTrackingPlansNext] = useState("")
  const [personalTrackingPlansLoading, setPersonalTrackingPlansLoading] = useState(false)
  const [personalTrackingPlans, setPersonalTrackingPlans] = useState<Array<TrackingPlan>>([])
  ...
  const getPersonalTrackingPlans = async (refresh = false) => {
    try {
      ...
      if (!refresh) {
        ...
        setPersonalTrackingPlans([...personalTrackingPlans, ...(resp?.items || [])])
      } else {
        setPersonalTrackingPlansNext(resp?.pagination?.token || "")
    ...
    <>
      <div className="">
        <div className="flex items-center justify-between">
          ...
          {personalTrackingPlansLoading && <Spinner tailwindBorderColor={"border-servian-black dark:border-servian-white"}/>}
        </div>
        ...
        <div className="flex items-center space-x-4 overflow-x-scroll pb-6">
          <NewTrackingPlanButton onClick={toggleNewTrackingPlanModal} />
          {
            [...personalTrackingPlans].map((item, index) => (
              <TrackingPlanTileContainer key={index} onClick={() => rounter.push(`/tracking-plan/${item.tracking_plan_uuid}`)}>
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">{moment(item.create_at_iso).format("DD.MM.YYYY")}</p>
